  ...
  const connectWallet = async () => {
    try {
      ...
      if (!error.code == '-32002') {
        ...
        alert(
          'Open Metamask to confirm that there no pending connection request.'
        );
      }
    }
  };
  ...
  const vContract = async () => {
    try {
      ...
      if (ethereum) {
        ...
        // console.log('Retrieved total wave count...:  ', vCount);

        // const visits = await VPortalContract.getTotalWaves();
        // setVisits(visits)
        // visits = visits.map(visit => <li key={visit}>{visit}</li>)
      } else {
  ...
  useEffect(async () => {
    ...
    const onNewWave = (from, wavedAt, message)=>{
      ...
      ...prevState,
      {
        ...
        wavedAt: new Date(timestamp * 1000),
        messages: message,
      },
    ]);
    }
            ...
            <div className="p-16">
              <div className="sm:text-center lg:text-left">
                ...
                <h2 className="mt-4 font-bold tracking-tight text-gray-400">
                  The Code-Blooded Technical Product Manager that bridges the gap between the Product and the Technology. 
                  <br />
        ...
        <div className="w-full bg-gray-700 py-16 ">
          ...
          {visits.map((visitor) => (
            <li className="flex p-12 mx-auto max-w-6xl gap-4 bg-gray-800 text-gray-400">
              <div className="bg-gray-700 w-32 h-32 p-4 flex place-items-center">
                {visitor.waveNo}  👋
              </div>
